...
// ---------------------------------------------------------------- 石狮
/**
 * 石狮（六段法，移植自 ps1 VxLion）：须弥座 → 身 → 胸 → 头 → 卷毛凸点 → 尾。
...
export function lion(w, p) {
  ...
  box(w, xLeft + 1, y0 + 1, z0, 8, 1, dz, '砂岩·亮');              // 座上枋
  box(w, xLeft + 2, y0 + 3, z0, 6, 5, dz, '砂岩·亮');              // 身
  box(w, xLeft + 2, y0 + 3, z0, 2, 3, dz, '砂岩·暗');              // 前腿暗面
  box(w, xLeft + 3, y0 + 7, z0, 4, 4, dz, '砂岩·亮');              // 头
  // 卷毛凸点：ps1 为绝对 y(15/12)，此处改为相对 y0 偏移(+11/+8)，使狮子可整体上下平移
  const tufts = [[3, 11], [6, 11], [4, 11], [3, 8], [6, 8]];
  for (const [dx, dy] of tufts) box(w, xLeft + dx, y0 + dy, z0, 1, 1, dz, '砂岩·暗');
  box(w, xLeft + 7, y0 + 5, z0, 1, 2, dz, '砂岩·暗');              // 尾
  return w;
}
