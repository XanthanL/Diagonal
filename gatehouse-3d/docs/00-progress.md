...
#### 任务卡 T9 —— P4 第一重檐 + 斗栱带 ✅
- **内容**：跨度最大的一层檐：如意斗栱程序化带 → 檐口瓦当线 → 三段举折坡面 → 两端翘角 → 角尖凤头。
- **依据**：02 §4.1/§4.2/§5；roof-profile-guide.png。
- **验收**：[x] 檐口跨＝elevation 源 x24..135（中央平段 46..113）；[x] 斗栱金点节奏＝周期 8（2 宽金点 x=28..132，与 gen-samples.ps1 T1 段一致）；[x] 翘角弧线上扬方向正确（向外向上）。
- **勘误（本轮记）**：原卡「檐口跨约 x44–116」「3 格周期」「3 步」为 elevation-guide 六轮复校**之前**旧数（复校后＝rise 平滑弧 + 周期 8），以 02 §4 修订与 ps1 现源为准。
- **完成注记（2026-08-29 验收轮）**：`parts/eave1.js`+`spec.TIERS[0]` 逐项对源核验 1:1 等价（eave {80,56,34,46,12,5} = ps1 `VxEaveBand 80 56 34 46 12 5`）；斜角特写 `check-t9-eave1.png` 目检通过。**更正 T21 遗留①**：raycast 证明"悬浮条片"=GridHelper 网格线+翘角薄尾正常观感，部件无伪影，T13 撤销该项。selftests 15/15。下一任务 **T10**。

#### 任务卡 T10 —— P5 二三层墙身（匾额·花板）✅
- **内容**：二层黑漆墙身（大匾 + 金字行 + 双花板列）、三层墙身（中央 + 两翼 D4 对称花板）。
- **依据**：02 §6.1/§6.4；照片 gate-front-01 匾额区、carving-roundel-sample.png。
- **验收**：[x] 大匾 y52..58 位于一重檐（顶 46）与三重檐墙（起 62）之间的 T2 檐带正中、被檐体包裹；[x] 花板为 D4 中心对称构图（外金框 + 内金框 + 中心菱形 + 花心）；[x] 程序化棂条存在于两翼/次间 lattice()（pitchX/pitchY 生成），非本层构件。
- **勘误（本轮记）**：原卡「团窠＝八边近似圆环」「三层中央木格窗」为复校第三/五轮改版前旧词——圆环方案已废弃改 D4 花板（见 §七 第三轮），格窗按现源归两翼次间。
- **完成注记（2026-08-29 验收轮）**：`walls.js`（T2/T3 黑漆墙身，顶格 yTop−h 贴合带底）+ `upperRoofs.js`（T2..T4 带与嵌板同世界绘制；plaque/panelSym 参数逐项 = ps1 `VxPanelSym 58 55 12 7 / 80 79 8 9 / 57 79 6 7 / 80 101 11 11`）。特写 `check-t10-wallband.png` 目检：匾在带心、上下檐体包裹、左右花板对称，与 elevation 一致。selftests 15/15。下一任务 **T11**。
...
- **验收**：[x] 正侧 90° 视角截图有层次（柱排可数、山面有构造）；[x] 三角 ≤250k；[x] selftest 全绿。
- **完成注记（2026-08-29 执行轮）**：`spec.Z` 增 COL_FRONT/COL_MID/COL_BACK/WALL_FRONT/WALL_BACK/BEAM 分层常量，BODY 18→23 格（≈4.6 m），EAVE_BAND/FACE 随扩 [6,35]/[36,37]；`gatehouse.js` 重写为「三排柱（均落砂岩柱础）+ 双皮墙壳 + 山墙条 + 柱头枋 + 门槛石」，门叶/彩画带/大匾各归前皮层面，eave1/walls/upperRoofs/wings 经 spec 常量自动获得进深。进深加大使三角越预算，附带 **builder 底面优化**：多层世界最低层 -Y 底面（永远不可见）不再发射，selftest #3 期望 54→45 同步；实测 tris≈245k≤250k、selftests 15/15、#11 借门槛石/柱础接地。截图 check-t22-side / check-t22-front 入库。诚实备注：山花板与悬山两侧搏风未做（归 T11 脊饰验收）；侧视柱排层次在中景可辨、近景细部（枋头出跳）从简。
...
| T9 | 第一重檐+斗栱带 | C | ✅（2026-08-29 验收轮，卡片旧数已勘误） |
| T10 | 二三层墙身 | C | ✅（2026-08-29 验收轮，卡片两项勘误） |
| T11 | 二~四重檐+脊饰 | C | ☐ |
| T12 | 次间与两翼 | C | ☐ |
| T13 | 合并比对（M2 验收） | C | ☐ |
| T14 | 聚焦系统 | D | ☐ |
...
| 2026-08-29（T22 执行轮） | 应用户「继续」 | **T22 ✅**：spec 增 Z 分层常量（COL_FRONT/MID/BACK、WALL_FRONT/BACK、BEAM），BODY 18→23 格、檐带随扩；gatehouse 重写为三排柱+双皮墙壳+山墙条+柱头枋+门槛石；连带 builder 最低层底面省去优化（多层世界），selftest #3 期望同步 54→45；实测 tris≈245k、15/15 全绿、#11/#14 通过；check-t22-side / check-t22-front 入库。山花板/搏风归 T11。下一任务 **T9**。 |
| 2026-08-29（T9 验收轮） | 应用户「继续」 | **T9 ✅**：`parts/eave1.js`+`spec.TIERS[0]` 与 gen-samples.ps1 T1 段逐项对源等价（带体/金点周期/举折/翘角），斜角特写 check-t9-eave1 目检通过；原卡"x44–116 / 3 格周期 / 3 步翘角"为六轮复校前旧数，**记勘误**（验收改以现源为准）。raycast 澄清：T21 遗留①"悬浮条片"=GridHelper 网格线与翘角薄尾观感，非部件伪影，撤销该遗留。selftests 15/15。下一任务 **T10**。 |
| 2026-08-29（T10 验收轮） | 应用户「继续」 | **T10 ✅**：`walls.js`+`upperRoofs.js` 与 ps1 T2/T3/T4 段逐项对源等价（匾 y52..58 chars9、VxPanelSym 四型尺寸全同）；特写 check-t10-wallband 目检过。原卡「八边近似圆环」「三层木格窗」为复校前旧词，**记勘误**（现源＝D4 花板；格窗在两翼次间）。selftests 15/15。下一任务 **T11**。 |
| 2026-08-29（檐角牛角修正轮） | 用户指出建模重大错误：檐端应是自粗渐细、向上挑的尖角（牛角形），此前做成等深板状尾 | 重写 `ops.eaveBand` 角部 z 向深度按 (1−u)^1.15 幂曲线向中轴收分、`ops.eaveTip` 三段尾深 3→2→1 抬升收金尖；#11 一度浮空 16 格（尾段共行不足）→ 改逐段共行修复；02 §4.2 立「牛角铁律」并删挂风铃旧述。selftests 15/15、check-horn-eaves.png 入库。 |
