...
- **内容**：三重檐逐级收窄（角部为牛角弧线起翘，见 §4.2）、顶层收分、灰塑正脊 + 鸱吻 + 立体宝顶。
- **依据**：02 §4.1/§4.2/§4.3（脊部原型参考见内部档案）。
- **验收**：[x] 四重檐宽度严格递减（`#12` 断言机器校验 T1..T4 halfW 56→44→38→30）；[x] 正脊灰塑·亮 + 宝顶 16→12→8→4 阶梯居中 x=80（端正）；[x] 总高 y_max=117＝118 格，落 elevation 标线（#10 断言守护）。
- **勘误（本轮记）**：原卡「翘角步数 2→2→1、风铃」为檐角牛角化（§4.2 铁律）与"实拍无风铃"复校前旧词，已废。
- **完成注记（2026-08-29 验收轮）**：`upperRoofs.js` 的 T2/T3/T4 `eaveTier` + `ridgeSet` 与 gen-samples.ps1 顶段**逐格 1:1 对表**（顶柱 x72..87 y108..111、正脊 x56..103 y112..113、鸱吻四段、宝顶 16→12→8→4、基座 x70..89 金嵌 x72/78/84/90）。selftests 15/15。**发现 bug 记 T14**：data.js 中 part-06「上层檐与脊」的 cam/target 锚点指向空地（聚焦后画面无物），T14 聚焦系统联测时修锚点表；本轮改用世界包围盒临时取景核验（`t11-top` 视角，宝顶/瓦垄/金尖翘角齐备）。诚实备注：收细后的背侧翘角尖在斜视角下偏薄、远看近似浮板，#11 已证 6-连通接地，最终以 T13 正视剪影为准。
...
| T11 | 二~四重檐+脊饰 | C | ✅（2026-08-29 验收轮；06 锚点 bug 记 T14） |
| T12 | 次间与两翼 | C | ☐ |
| T13 | 合并比对（M2 验收） | C | ☐ |
| T14 | 聚焦系统 | D | ☐ |
| T15 | HUD/双语/导览 | D | ☐ |
...
| 2026-08-29（T11 验收轮） | 应用户「继续 T11」 | **T11 ✅**：`upperRoofs.js`（T2..T4 eaveTier + ridgeSet）与 gen-samples.ps1 顶段逐格 1:1（顶柱 x72..87 y108..111、正脊 x56..103 y112..113、鸱吻四段、宝顶 16→12→8→4、基座金嵌 x72/78/84/90）；#10 总高 y_max=117、#12 四重檐 56>44>38>30、#14 无重叠，selftests 15/15。原卡"翘角步数 2→2→1 / 风铃"为牛角化+去风铃前旧词，记勘误。Bug 记 T14：data.js part-06「上层檐与脊」cam/target 锚点指向空地（聚焦无物），本轮改以世界 bbox 临时取景核验（check-t11-ridge.png）。背侧翘角斜视偏薄属牛角收细正常结果，T13 正视剪影终判。下一任务 **T12**。 |
