    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="absolute top-0 right-0 h-full w-[380px] max-w-[90vw] panel z-30 overflow-y-auto shadow-lift"
        >
          <div className="sticky top-0 panel border-b border-line-soft px-4 py-3 flex items-center justify-between bg-white/90">
            <div className="accent-line pl-3">
              <div className="label-eyebrow">STAGE {String(stage.index + 1).padStart(2, "0")} / 05</div>
              <h2 className="text-lg font-semibold text-ink-900">{lang === "zh" ? stage.name : stage.nameEn}</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="px-4 py-4 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2 text-xs flex-wrap">
                ...
                <span className="px-2 py-1 rounded bg-paper-200 text-ink-700 border border-line-soft">{stage.output}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {stage.principle.map((p, i) => (
                  <p key={i} className="text-[13px] leading-relaxed text-ink-700">{p}</p>
                ))}
              </div>
            </div>
                    ...
                    {stage.params.map((p, i) => (
                      <tr key={i} className={i % 2 ? "bg-paper-100" : "bg-white"}>
                        <td className="px-3 py-2 text-ink-600">
                          ...
                          {p.indicative && <span className="ml-1 text-[9px] text-amber-500 align-top">参考</span>}
                        </td>
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="panel-solid rounded-2xl max-w-2xl w-full p-6 md:p-8 max-h-[85vh] overflow-y-auto shadow-lift"
          >
            <div className="flex items-start justify-between mb-4">
              <div className="accent-line pl-3">
                <div className="label-eyebrow">自贡 · ZIGONG</div>
                <h2 className="text-xl md:text-2xl font-semibold text-ink-900">{zigongGeology.title}</h2>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="space-y-3">
              {zigongGeology.paragraphs.map((p, i) => (
                <p key={i} className="text-sm leading-relaxed text-ink-700">{p}</p>
              ))}
            </div>
